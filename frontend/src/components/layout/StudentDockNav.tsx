...
const navItems: NavItem[] = [
  ...
  { icon: 'explore', label: 'Universities', href: '/universities' },
  { icon: 'school', label: 'Mentorship', href: '/mentorship' },
  { icon: 'chat', label: 'Messages', href: '/student-messages' },
  { icon: 'person', label: 'Profile', href: '/student-profile' },
  { icon: 'settings', label: 'Settings', href: '/student-settings' }
];

export function StudentDockNav() {
  const pathname = usePathname();

  // Hide nav on the setup/onboarding flow
  if (pathname === '/student-setup') {
    return null;
  }
